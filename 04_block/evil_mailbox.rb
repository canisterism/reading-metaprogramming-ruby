...
class EvilMailbox
  def initialize(source, secret_string = nil)
    @source = source
    if secret_string
      source.auth(secret_string)
      store_secret(secret_string)
    end
  end

  def send_mail(to, body)
    pp secret
    result = @source.send_mail(to, "#{body}#{secret}")
  ...
  def store_secret(secret_string)
    @secret = Base64.encode64(secret_string)
  end
end
